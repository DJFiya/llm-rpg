...
```bash
python -m llm_rpg.main
```

Or, after a `pip install -e .`, use the console script:
...
Slash commands: `/look` (re-describe), `/map` (show discovered map), `/help`,
`/quit` (save and exit). Saves are stored as one SQLite file per run in
`saves/` and offered on the next launch.
